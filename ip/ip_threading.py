    ...
    def soup(self,url,number):
        ...
        while num < len(td_soup):
            if num%8 == 0 :
                ...
                ip.execute("INSERT INTO IPS (ID,IP,PORT) VALUES ('%d','%s','%s')"%(number,td_soup[num].get_text(),td_soup[num+1].get_text()));
                number += 1
                conn.commit()
            else:
                pass
    ...
    def start(self):
        ...
        for i in range(10):
            if i == 0:
                ...
                t = threading.Thread(target=self.run,args=(number,url))
                threads.append(t)
            else:
                number = number*10
