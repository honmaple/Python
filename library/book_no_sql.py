...
# print(soup.a)
# for item in item_indro.stripped_strings:
    # if i < 20:
        # print(item)
        # i +=1
# item_indro = item_soup.find_all(re.compile("<dt.*?>.*?<dt>"))
...
try:
    ...
    print ( e )
    sys.exit()
    # for string in soup.stripped_strings:   #输出的字符串中可能包含了很多空格或空行,使用 .stripped_strings 可以去除多余空白内容:
        # print(repr(string))
    # main_a = main_soup.find_all('a',limit=23)
