...
class Novel:
    ...
    def getPage(self,url):
        try:
            ...
            # pattern = re.compile('<div id="p_content.*?">.*?<cc.*?>(.*?)</cc>',re.S)
            #将页面转化为UTF-8编码
    ...
    def getChioce1(self,url):
        ...
        #利用urlopen获取页面代码
        response = urllib2.urlopen(request)
        pattern = re.compile('<div class="novel-post-content">.*?</div>.*?<div class="novel-post-bottom.*?>',re.S)
        #将页面转化为UTF-8编码
        content = response.read().decode('utf-8')
        items = re.findall(pattern,content)
        for item in items:
